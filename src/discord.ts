...
export async function sendDiscordMessage(config: INeonConfig, content: string): Promise<IDiscordSendResult> {
  ...
  const result: IDiscordSendResult = {
    id,
    channelId,
...
async function postDiscordMessage(channelId: string, token: string, content: string): Promise<unknown> {
...
function extractDiscordMessageId(payload: unknown): string {
...
function extractDiscordError(payload: unknown): string | undefined {
  if (!isRecord(payload)) {
    return undefined;
  }

...
export function memoryDatabasePath(config: INeonConfig): string {
  return join(config.stateDir, "memory", "memory.sqlite");
}

export async function initMemoryDatabase(config: INeonConfig): Promise<IMemoryDatabaseInfo> {
...
export async function createMemory(config: INeonConfig, input: ICreateMemoryInput): Promise<IMemoryRecord> {
...
export async function listMemories(config: INeonConfig, limit = DEFAULT_MEMORY_LIMIT): Promise<IMemoryRecord[]> {
  const db = await openMemoryDatabase(config);
  try {
    migrate(db);
...
export async function searchMemories(config: INeonConfig, query: string, limit = DEFAULT_MEMORY_LIMIT): Promise<IMemorySearchResult[]> {
...
async function openMemoryDatabase(config: INeonConfig): Promise<DatabaseSync> {
  await ensureDir(join(config.stateDir, "memory"));
...
function indexMemory(db: DatabaseSync, id: string): void {
...
function parseMemoryRow(row: Record<string, unknown>): IMemoryRecord {
  return {

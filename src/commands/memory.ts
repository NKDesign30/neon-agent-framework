...
import { createMemory, initMemoryDatabase, listMemories, searchMemories, type IMemoryRecord, type IMemorySearchResult } from "../memoryStore.js";
import { readBooleanFlag, readStringFlag, type ICliFlags } from "../args.js";

export async function runMemory(flags: ICliFlags): Promise<void> {
...
function printMemoryResult(flags: ICliFlags, memory: IMemoryRecord): void {
  if (readBooleanFlag(flags, "json")) {
...
function printMemoryList(flags: ICliFlags, memories: readonly IMemoryRecord[]): void {
  if (readBooleanFlag(flags, "json")) {
...
function printMemorySearch(flags: ICliFlags, results: readonly IMemorySearchResult[]): void {
  if (readBooleanFlag(flags, "json")) {
...
function readLimit(flags: ICliFlags): number {
